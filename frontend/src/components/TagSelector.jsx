...
export default function TagSelector({ allTags, selectedTagIds, onToggleTag, onCreateTag }) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="w-64 p-3 bg-white border-[var(--n-border)] rounded-xl shadow-lg" align="start">
        <div data-testid="tag-selector-content">
          <p className="text-xs font-medium text-[var(--n-text-secondary)] mb-2">Select tags</p>
          {allTags.length > 0 && (
              ...
              {allTags.map((tag) => {
                ...
                return (
                  <button
                    ...
                    } ${isSelected ? 'ring-2 ring-[var(--n-text)] ring-offset-1' : 'opacity-60 hover:opacity-100'}`}
                  >
                    {tag.name}
                  </button>
                );
          ...
          <div className="border-t border-[var(--n-border)] pt-2.5">
            <p className="text-xs font-medium text-[var(--n-text-secondary)] mb-2">Create new tag</p>
            <div className="flex gap-1.5 mb-2">
              {COLOR_OPTIONS.map((color) => (
                <button
                  ...
                  } ${newTagColor === color ? 'ring-2 ring-[var(--n-text)] ring-offset-1' : ''}`}
                />
              ))}
            </div>
            <div className="flex gap-1.5">
              <input
                ...
                className="flex-1 text-xs px-2 py-1.5 rounded-md border border-[var(--n-border)] bg-transparent placeholder-[var(--n-text-secondary)] focus:outline-none focus:ring-1 focus:ring-[#EBEBEA]"
              />
              <button
                data-testid="create-tag-btn"
                onClick={handleCreate}
                disabled={!newTagName.trim()}
                className="px-2 py-1.5 text-xs font-medium rounded-md bg-[var(--n-text-title)] text-white hover:bg-[var(--n-text)] disabled:opacity-30 transition-colors"
              >
